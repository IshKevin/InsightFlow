...
class TestAuth:
    ...
    def test_login_missing_fields(self, base_url):
        resp = requests.post(f'{base_url}/api/auth/login', json={'email': 'admin@amalitech.com'})
        assert resp.status_code == 400
...
class TestDataSources:
    ...
    def test_create_datasource(self, base_url, auth_headers):
        resp = requests.post(
            ...
            json={'name': 'QA Test Source', 'type': 'CSV', 'file_path': '/tmp/qa_test.csv'},
        )
    ...
    def test_get_datasource_by_id(self, base_url, auth_headers):
        ...
        resp = requests.get(f'{base_url}/api/datasources/{ds_id}/', headers=auth_headers)
        assert resp.status_code == 200
...
class TestPipelines:
    ...
    def datasource_id(self, base_url, auth_headers):
        resp = requests.post(
            f'{base_url}/api/datasources/',
            headers=auth_headers,
            json={'name': 'Pipeline Test Source', 'type': 'CSV', 'file_path': '/tmp/pipeline.csv'},
        )
        assert resp.status_code == 201
        return resp.json()['id']
